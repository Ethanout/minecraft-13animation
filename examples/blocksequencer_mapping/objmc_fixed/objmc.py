...
parser.add_argument(
    "--head-carrier", action="store_true",
    help="Use the humanoid head pivot and yaw-only carrier (requires the matching shader)",
)
parser.add_argument("--texs", help="Specify a texture file", nargs="*", default=texs)
parser.add_argument(
    "--out", type=str, help="Output json and png", nargs=2, default=output
)
parser.add_argument(
    "--offset", type=float, help="Positional offset of model", nargs=3, default=offset
)
parser.add_argument("--scale", type=float, help="Scale of model", default=scale)
parser.add_argument(
    "--duration", type=int, help="Duration of the animation in ticks", default=duration
)
parser.add_argument(
    "--easing",
    type=int,
...
parser.add_argument(
    ...
    action="store_true",
    dest="autoplay",
    help="Always interpolate animation, colorbehavior of all 'time' overrides this",
)
parser.add_argument(
    "--visibility",
    type=int,
...
parser.add_argument(
    "--nopow", action="store_true", dest="nopow", help="Disable power of two textures"
)
parser.add_argument(
...
def getargs(args):
    global head_carrier
    head_carrier = args.head_carrier
    global objs
    global texs
    global output
    global offset
    ...
    for line in obj:
        ...
        if line.startswith("f "):
            d["faces"].append(
                tuple(
                    [
                        [int(i) - 1 for i in vert.split("/")]
                        for vert in " ".join(line.split()).split(" ")[1:]
                    ]
                )
            )
    obj.close()
...
def getheader(out, faceid, x, y, ty):
    ...
    out.putpixel(
        (posx, posy),
        (int(posx / 256) % 256, posx % 256, int(posy / 256) % 256, posy % 256),
    )
    return [
        (posx + 0.1) * 16 / x,
        (posy + 0.1) * 16 / ty,
...
def newelement(out, index, x, y, ty):
    ...
    if head_carrier:
        # The head layer translates by -0.25, then scales Y by -0.625.
        # Item rendering subtracts 0.5. Thus Y=1.6/16 cancels that offset.
        # Keep a nonzero face for baking its east normal. The carrier radius
        # is less than 0.00001 blocks before the entity scale is applied.
        cube["from"] = [8, 1.5999, 7.9999]
        cube["to"] = [8, 1.6001, 8.0001]
        cube["faces"] = {"east": cube["faces"]["north"]}
...
def objmc(
    ...
    if (ty > 4096 and x < 4096) or (ty > 8 * x):
        print(
            col.warn
            + "output height may be too high, consider increasing width of input texture or reducing number of frames to bring the output texture closer to a square."
            + col.end
        )
    ...
    # initial info
    print("%\033[K", end="\r")
    print("compression enabled:", compression_enabled)
    print(
        "faces: ",
        nfaces,
        ", verts: ",
    ...
    # 0: marker
    out.putpixel((0, 0), (12, 34, 56, 79 if compression_enabled else 78))
    # 1: texsize
    ...
    out.putpixel(
        (2, 0),
        (
            int(nvertices / 16777216) % 256,
            int(nvertices / 65536) % 256,
            int(nvertices / 256) % 256,
            nvertices % 256,
        ),
    )
    # 3: nobjs, ntexs
    ...
    out.putpixel(
        (4, 0),
        (
            int(duration / 65536) % 256,
            int(duration / 256) % 256,
            duration % 256,
            128 + (int(autoplay) << 6) + (easing << 4) + (interpolation << 2),
        ),
    )
    # 5: data heights
    out.putpixel(
        (5, 0),
        (
            int(vpheight / 256) % 256,
            int(vpheight) % 256,
            int(vtheight / 256) % 256,
            vtheight % 256,
        ),
    )
    ...
    if head_carrier:
        out.putpixel((7, 0), (72, 69, 65, 68))  # HEAD protocol marker
...
def openobjs():
    f = tkfd.askopenfilenames(
        initialdir=path, title="Select Obj Files", filetypes=[("Obj Files", ".obj")]
    )
    global objs
    objs = list(f)
    f = [os.path.basename(f) for f in f]
...
if not len(sys.argv) > 1:
    ...
    # obj list
    tk.Button(window, text="Select Objs", command=openobjs, borderwidth=3).grid(
        column=0, row=0, padx=(5, 20), pady=(5, 0), sticky="NEW"
    )
    objlist = tkst.ScrolledText(window, height=500)
    objlist.grid(column=0, row=1, rowspan=3, sticky="NEW", padx=5)
    # tex list
    tk.Button(window, text="Select Textures", command=opentex, borderwidth=3).grid(
        column=1, row=0, padx=(5, 65), pady=(5, 0), sticky="NEW"
    )
    fu = tk.BooleanVar()
    tk.Checkbutton(window, text="Flip UV", variable=fu).grid(
        column=1, row=0, sticky="NES"
    )
    texlist = tkst.ScrolledText(window, height=500)
    texlist.grid(column=1, row=1, rowspan=3, sticky="NEW", padx=5)

    # options
    ttk.Separator(window, orient=tk.VERTICAL).grid(
        column=1, row=0, rowspan=4, sticky="NSE"
    )
    ...
    sc = tk.StringVar()
    Floatbox(basic, width=17, textvariable=sc).grid(
        column=1, row=1, columnspan=3, sticky="NESW"
    )
    # noshadow
    ns = tk.BooleanVar()
    tk.Checkbutton(basic, text="No Shadow", variable=ns).grid(
        column=0, row=2, columnspan=4, padx=(20, 0)
    )
    # autorotate
    ar = tk.StringVar()
    ...
    tk.Label(advanced, text="Animation").grid(
        column=0, row=0, columnspan=3, sticky="EW", pady=(0, 5)
    )
    ttk.Separator(advanced, orient=tk.HORIZONTAL).grid(
        column=0, row=0, columnspan=3, sticky="NEW"
    )
    ttk.Separator(advanced, orient=tk.HORIZONTAL).grid(
        column=0, row=0, columnspan=3, sticky="NEW", pady=(25, 5)
    )
    # duration
    tk.Label(advanced, text="Total Duration:").grid(
        column=0, row=1, sticky="W", padx=(5, 0)
    )
    dur = tk.StringVar()
    Number(advanced, textvariable=dur, width=5).grid(
        column=2, row=1, sticky="EW", padx=(0, 30)
    )
    tk.Label(advanced, text="ticks").grid(column=2, row=1, sticky="E", padx=(0, 15))
    # easing
    tk.Label(advanced, text="Position Easing:").grid(
        column=0, row=2, sticky="W", padx=(5, 0), pady=(2, 0)
    )
    ea = tk.StringVar()
    earr = ["None", "Linear", "Cubic", "Bezier"]
    ttk.Combobox(
        advanced, values=earr, textvariable=ea, state="readonly", width=8
    ).grid(column=2, row=2, pady=(2, 0), sticky="W")
    # interpolation
    tk.Label(advanced, text="Texture Interpolation:").grid(
        column=0, row=3, sticky="W", padx=(5, 5), pady=(2, 0)
    )
    it = tk.StringVar()
    iarr = ["None", "Linear"]
    ttk.Combobox(
        advanced, values=iarr, textvariable=it, state="readonly", width=8
    ).grid(column=2, row=3, padx=(0, 5), pady=(2, 0), sticky="W")
    # autoplay
    ap = tk.BooleanVar()
    tk.Checkbutton(advanced, text="Auto Play", variable=ap).grid(
        column=0, row=4, columnspan=3, sticky="N"
    )
    # color behavior
    ...
    ).grid(column=0, row=6, columnspan=3, padx=(0, 125))
    ttk.Combobox(
        advanced, values=cbarr, textvariable=cb[1], width=7, state="readonly"
    ).grid(column=0, row=6, columnspan=3, padx=(0, 0))
    ttk.Combobox(
        advanced, values=cbarr, textvariable=cb[2], width=7, state="readonly"
    ).grid(column=0, row=6, columnspan=3, padx=(125, 0))
    ...
    tk.Label(outfooter, text="Output").grid(
        column=0, row=0, columnspan=3, sticky="EW", pady=(0, 10)
    )
    ttk.Separator(outfooter, orient=tk.HORIZONTAL).grid(
        column=0, row=0, columnspan=3, sticky="NEW"
    )
    ttk.Separator(outfooter, orient=tk.HORIZONTAL).grid(
        column=0, row=0, columnspan=3, sticky="SEW", pady=(25, 10)
    )
    outjson = tk.StringVar()
    outpng = tk.StringVar()
    tk.Entry(outfooter, textvariable=outjson, width=10).grid(
        column=0, row=2, columnspan=3, padx=10, sticky="NEW"
    )
    tk.Label(outfooter, text=".json").grid(column=2, row=2, padx=10, sticky="NE")
    tk.Entry(outfooter, textvariable=outpng, width=10).grid(
        column=0, row=3, columnspan=3, padx=10, sticky="NEW"
    )
    ...
    tk.Button(window, text="←", command=prev).grid(
        column=0, columnspan=3, row=4, sticky="W", padx=(60, 0), pady=5
    )
    tk.Button(window, text="→", command=next).grid(
        column=0, columnspan=3, row=4, sticky="W", padx=(80, 0), pady=5
    )
    tk.Button(window, text="Save History", command=savehistory).grid(
        column=0, columnspan=3, row=4, padx=(0, 200), pady=5
    )
    tk.Button(window, text="Load History", command=loadhistory).grid(
        column=0, columnspan=3, row=4, padx=(0, 47), pady=5
    )
    tk.Button(window, text="Run History", command=runhistory).grid(
        column=0, columnspan=3, row=4, padx=(102, 0), pady=5
    )
    ttk.Separator(window, orient=tk.HORIZONTAL).grid(
