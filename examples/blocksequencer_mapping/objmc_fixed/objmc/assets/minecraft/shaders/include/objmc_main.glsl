...
if (marker == ivec4(12,34,56,78) || marker == ivec4(12,34,56,79)) {
    ...
    bool headCarrier = t[7] == ivec4(72, 69, 65, 68);
    //1: texsize
    ...
    isHand = headCarrier ? 0 : int(ishand(ProjMat));
...
//custom entity rotation
        ...
        if (isHand + isGUI == 0) {
            if (headCarrier) {
                // Each tiny carrier vertex already contains the head pivot.
                // Its east normal follows yaw and stays fixed under pitch.
                // No value from another vertex invocation is required.
                entityAnchor = Pos;
                float yaw = atan(Normal.z, Normal.x);
                posoffset = rotate(rotation + vec3(0, yaw, 0)) * posoffset;
            } else {
            // The OBJ vertices are offsets from the carrier item quad. The
            // carrier origin must come from the quad subgroup; using
            // `Pos - Position` here is always zero because Pos is initialized
            // from Position at the start of the vertex shader.
            entityAnchor = subgroupQuadBroadcast(Pos, 2);
            float entityScale = 1.0;
            if (any(greaterThan(autorotate,vec2(0)))) {
                // Normal is transformed by VertexConsumer with the entity
                // pose. Use only its horizontal projection. The vertical
                // component is deliberately discarded so a head carrier
                // never follows player pitch.
